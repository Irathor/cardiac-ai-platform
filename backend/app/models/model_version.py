...
class ModelVersion(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    `prototypes` is the actual model artifact this platform's inference path
    consumes (see app.services.analysis_service) — a plain JSON copy of what
    was logged to MLflow at `mlflow_run_id`/`mlflow_model_uri`, kept here so
    a live analysis request never needs to fetch an artifact from MLflow (see
    docs/architecture.md: MLflow is the source of truth for training
    history, but the request/response path must stay independent of it).
    ...
    prototypes: Mapped[dict] = mapped_column(JSON())
